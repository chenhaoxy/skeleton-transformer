...
def text_prompt():
    ...
    for ii, txt in enumerate(text_aug):
        text_dict[ii] = torch.cat([device.tokenize(txt.format(c)) for c in label_text_map])
...
def text_prompt_openai_random():
    ...
    for pasta_list in paste_text_map0:
        ...
        for item in pasta_list:
            temp_list.append(device.tokenize(item))
...
def text_prompt_openai_pasta_pool_4part():
    ...
    for ii in range(num_text_aug):
        if ii == 0:
            text_dict[ii] = torch.cat([device.tokenize((pasta_list[ii])) for pasta_list in paste_text_map2])
        elif ii == 1:
            text_dict[ii] = torch.cat([device.tokenize((','.join(pasta_list[0:2]))) for pasta_list in paste_text_map2])
        elif ii == 2:
            text_dict[ii] = torch.cat([device.tokenize((pasta_list[0] +','.join(pasta_list[2:4]))) for pasta_list in paste_text_map2])
        elif ii == 3:
            text_dict[ii] = torch.cat([device.tokenize((pasta_list[0] +','+ pasta_list[4])) for pasta_list in paste_text_map2])
        else:
            text_dict[ii] = torch.cat([device.tokenize((pasta_list[0]+','+','.join(pasta_list[5:]))) for pasta_list in paste_text_map2])
...
def text_prompt_openai_pasta_pool_4part_ucla():
    ...
    for ii in range(num_text_aug):
        if ii == 0:
            text_dict[ii] = torch.cat([device.tokenize((pasta_list[ii])) for pasta_list in ucla_paste_text_map1])
        elif ii == 1:
            text_dict[ii] = torch.cat([device.tokenize((','.join(pasta_list[0:2]))) for pasta_list in ucla_paste_text_map1])
        elif ii == 2:
            text_dict[ii] = torch.cat([device.tokenize((pasta_list[0] +','.join(pasta_list[2:4]))) for pasta_list in ucla_paste_text_map1])
        elif ii == 3:
            text_dict[ii] = torch.cat([device.tokenize((pasta_list[0] +','+ pasta_list[4])) for pasta_list in ucla_paste_text_map1])
        else:
            text_dict[ii] = torch.cat([device.tokenize((pasta_list[0]+','+','.join(pasta_list[5:]))) for pasta_list in ucla_paste_text_map1])
